...
const MANDATORY_QUESTIONS_COUNT = 10;

const ConversationalOnboarding = ({ onSkip }) => {
  ...
  const handleSkip = () => {
    setSkipped(true);
    if (onSkip) {
      onSkip(answers, questions.slice(MANDATORY_QUESTIONS_COUNT));
    }
  };

  // Optionally, allow user to go back and answer skipped questions later
  const handleEdit = (editStep) => {
    setStep(editStep);
    setInput(answers[questions[editStep].id] || '');
  };
        ...
        >
          ...
          {step < questions.length ? (
            <>
              ...
              {/* Show skip button after 10th question if not already skipped */}
              {step === MANDATORY_QUESTIONS_COUNT - 1 && !skipped && (
                <button
                  className="chat-submit-btn"
                  style={{ background: '#e75480', marginTop: 12 }}
                  type="button"
                  onClick={handleSkip}
                >
                  Skip & Complete Later
